...
public class PromiseFlowDomainImpl implements PromiseFlowDomain {
    ...
    public PromiseFlow submitPromiseFlow(PromiseFlow promiseFlow) {
        if (promiseFlow.getAccountId() == null) {
            throw new PlatformException(BaseErrorCode.PARAM, "账号ID不能为空");
        }
        // Money 非 Number 无法用 @Positive, 金额为正的校验落此处
        if (promiseFlow.getAmount() == null || !promiseFlow.getAmount().greaterThanZero()) {
            throw new PlatformException(BaseErrorCode.PARAM, "保证金金额必须大于0");
        }
        // 读供应商全局配置决定是否跳过人工审核
        ...
        promiseFlow.setId(promiseFlowRepository.promiseFlowSave(promiseFlow));
        if (skip) {

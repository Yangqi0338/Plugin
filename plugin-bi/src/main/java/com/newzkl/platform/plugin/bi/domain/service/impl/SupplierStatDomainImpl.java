...
public class SupplierStatDomainImpl implements SupplierStatDomain {

    private final StatRealtimeRepository realtimeRepo;

    /** 供应商 HOME 总览(实时 SUM) */
    @Override
    public HomeOverviewRes home(SupplierHomeQuery query) {
    ...
    /** 供应商供货结算趋势(按月份) */
    @Override
    public List<SettleTrendItemRes> settleTrend(SupplierHomeQuery query) {
        ...
        List<SettleTrendItemRes> list = new ArrayList<>();
        if (countMap != null) {
            ...
            for (SupplierSettleTrendDO row : rows) {
                SettleTrendItemRes item = new SettleTrendItemRes();
